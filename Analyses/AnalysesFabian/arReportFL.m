...
%% Head
lp(fid, '\\nonstopmode');
lp(fid, '\\documentclass[10pt, oneside, fleqn, landscape]{article}');
...
lp(fid, '\\setlength{\\textheight}{16 cm}');
lp(fid, '\\setlength{\\textwidth}{22 cm}');
lp(fid, '\\setlength{\\topmargin}{-1.5 cm}');
lp(fid, '\\setlength{\\hoffset}{-6 cm}');
...
for jm=1:length(ar.model)
    ...
    arWaitbar(cncounter, nncounter, 'Writing PDF...'); cncounter = cncounter + 1;
    if(isfield(ar.model(jm), 'y') && ~isempty(ar.model(jm).y))
    ...
    for jplot=1:length(ar.model(jm).plot)
        ...
        if(isfield(ar.model(jm), 'data'))
            ...
            if(isfield(ar.model(jm), 'y') && ~isempty(ar.model(jm).y))
                ...
                qadd = zeros(1,length(ar.model(jm).data(jd).y));%0;
            end
            ...
            if(isfield(ar.model(jm), 'y') )
                ...
                qmod = [];
                for j=1:length(ar.model(jm).data(jd).y)
                    if(~qadd(j))
...
%                                 try
                                    qalreadyset = qalreadyset && isequal(arSym(unique({ar.model(jm).fp{qdyn}})), ...
...
strsym = arSym(str);
% vorher = char(strsym);
sstrsym = simplify(subs(strsym, sym('FOLDRAUS'), 1));
% nachher = char(sstrsym);
% if strcmp(vorher,nachher)~=1
%     fprintf('%s\n%s\n',vorher, nachher)
% end
sstrsym = subs(sstrsym, svarlist, sshortlist);
